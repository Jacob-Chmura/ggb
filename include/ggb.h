...
class FeatureStoreBuilder {
 public:
  ...
  virtual auto commit(std::optional<GraphTopology> graph = std::nullopt)
      -> void = 0;
};
...
class GGBFeatureStore;

class GGBFeatureStoreBuilder final : public FeatureStoreBuilder {
 public:
  explicit GGBFeatureStoreBuilder(const GGBConfig &cfg, GGBFeatureStore &parent)
      : cfg_(cfg), parent_(parent), out_file_(cfg.db_path, std::ios::binary) {}
...
class GGBFeatureStore final : public FeatureStore {
 public:
  explicit GGBFeatureStore(GGBConfig cfg) : cfg_(std::move(cfg)) {
  ...
  [[nodiscard]] auto create_builder()
    ...
    return std::make_unique<GGBFeatureStoreBuilder>(cfg_, *this);
  }
 ...
 private:
  ...
  void refresh_mapping() {
    ...
    if (fd == -1) return;  // Silent return: file might not exist yet
  ...
  friend class GGBFeatureStoreBuilder;
};

// Implement commit here to access GGBFeatureStore private members via friend
inline auto GGBFeatureStoreBuilder::commit(
    [[maybe_unused]] std::optional<GraphTopology> graph) -> void {
  if (out_file_.is_open()) {
    out_file_.flush();
    out_file_.close();
  }
  // Sync the parent store with the builder's local index and metadata
  parent_.key_to_byte_ = std::move(key_to_byte_);
  parent_.tensor_size_ = tensor_size_;
  parent_.refresh_mapping();
}

[[nodiscard]] inline auto create_ggb_store(const GGBConfig &cfg)
    -> std::unique_ptr<FeatureStore> {
  return std::make_unique<GGBFeatureStore>(cfg);
}
